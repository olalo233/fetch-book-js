...
import type { NovelRepository, ChapterRepository } from '../../infrastructure/repositories';
...
export function createNovelRouter({ novelRepository, chapterRepository }: RoutesDeps) {
  ...
  router.get('/:id', async (c) => {
    ...
    if (!novel) {
      return c.json({ success: false, error: { code: 'NOT_FOUND', message: 'Novel not found' } }, 404);
    }
  ...
  router.get('/:id/chapters/:chapterId', async (c) => {
    ...
    if (!chapter || chapter.novelId !== novelId) {
      return c.json({ success: false, error: { code: 'NOT_FOUND', message: 'Chapter not found' } }, 404);
    }
